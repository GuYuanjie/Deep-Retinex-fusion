...
def k_means(input_signal, center_num, threshold):
    ...
    while True:
        ...
        for n in range(center_num):
            ...
            len_input = len(input_signal_cp[temp])
            if len_input==0:
                len_input = 10000

            present_center[n] = sum(input_signal_cp[temp].astype(int)) / len_input
        # 根据当前聚类中心分类
        pixls_labels = classifer(input_signal_cp, present_center)
        # 计算上一次聚类中心与当前聚类中心的差异
        loss = loss_function(present_center, pre_centet)
        num = num + 1
        print("K-means segamentation——>step:"+ str(num) + "   Loss:" + str(loss), '\r', end='')
        # 当损失小于迭代阈值时，结束迭代
        if loss <= threshold:
            break
    return pixls_labels
